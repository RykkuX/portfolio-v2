          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16 lg:gap-y-20">
            {services.map((service, index) => (
              <div key={index} className="space-y-6">
                {/* Service Title */}
                <h3 className={`${typographyClasses.responsiveH4} ${colorClasses.primary} font-mono`}>
                  {service.title}
                </h3>
                ...
                {/* Service Description */}
                <p className={`${typographyClasses.responsiveBody} ${colorClasses.primary} leading-relaxed max-w-sm`}>
                  {service.description}
                </p>
              </div>
            ))}
